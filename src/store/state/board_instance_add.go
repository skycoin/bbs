...
func (bi *BoardInstance) NewContent(content *object.Content) (uint64, error) {
	...
	var goalSeq uint64
	e := bi.GetPack().Do(func(p *skyobject.Pack, h *ActivityHeader) error {
		...
		uaHash, has := h.Get(content.Creator)
		if !has {
			// Add user activity page if not exist.
			uaHash, e = aPage.AppendUserActivity(content.Creator)
			if e != nil {
				return e
			}
		}
		if e := aPage.AddToUserActivity(uaHash, content); e != nil {
			return e
		}
