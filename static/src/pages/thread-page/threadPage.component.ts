...
export class ThreadPageComponent implements OnInit {
  ...
  addThreadVote(mode: number, ev: Event) {
    ...
    data.append('of_thread', this.threadKey)
    data.append('value', mode.toString())
    data.append('creator', this.user.loginInfo.PublicKey)
    this.api.submission(this.api.threadVoteType, data).subscribe(voteRes => {
      if (voteRes.okay) {
  ...
  addUserVote(ev: Event, ofUser: string, mode: string) {
    ...
    const data = new FormData()
    data.append('of_board', this.boardKey)
    data.append('of_user', ofUser)
    data.append('value', mode.toString())
    data.append('creator', this.user.loginInfo.PublicKey)
    this.api.submission(this.api.userVoteType, data).subscribe(result => {
      if (result.okay) {
        this.userTag = '';
      }
      this.loading.close();
  ...
  addPostVote(mode: number, post: Post, ev: Event) {
    ...
    data.append('of_board', this.boardKey)
    data.append('of_thread', this.threadKey)
    data.append('of_post', post.header.hash)
    data.append('value', mode.toString())
    data.append('creator', this.user.loginInfo.PublicKey)
    this.api.submission(this.api.postVoteType, data).subscribe(res => {
    ...
    if (this.user.loginInfo) {
      ...
      this.pop.open(content).result.then((result) => {
        if (result) {
          ...
          data.append('body', this.common.replaceHtmlEnter(this.postForm.get('body').value))
          data.append('creator', this.user.loginInfo.PublicKey)
          this.loading.start();
          this.api.submission(this.api.postType, data).subscribe((res: ThreadPage) => {
