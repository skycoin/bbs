...
export class ConnectionService {
    ...
    getAllConnections() {
        return this.http.get(this.base_url + 'get_all').map((res: Response) => res.json()).catch(err => this.common.handleError(err));
    }

    addConnection(data: FormData) {
        return this.http.post(this.base_url + 'add', data).map((res: Response) => res.json()).catch(err => this.common.handleError(err));
    }
